...
class DevCycleManager: ObservableObject {
    ...
    @Published var featureAColorHex: String = "#ffffffff" // Default color (blue)
    ...
    func initialize() {
        do {
            ...
            let user = try DevCycleUser
                .builder()
                .customData(["HASHED_SRF_ID":"12345","ACCOUNT_TYPE":"employeeAcct"])
                .build()
    ...
    private func setupVariableUpdates() {
        ...
        let featureAColor = client.variable(key: "feature-a-color", defaultValue: "#FFFFFF")
        ...
        DispatchQueue.main.async {
            self.featureAColorHex = featureAColor.value
        ...
        featureAColor.onUpdate { [weak self] updatedValue in
            DispatchQueue.main.async {
                print("Feature 'feature-a-color' updated to: \(updatedValue)")
                self?.featureAColorHex = updatedValue
            }
        }
    }
}
